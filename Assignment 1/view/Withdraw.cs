...
namespace Assignment_1.view
{
    class Withdraw : View
    {
        ...
        private String[] Content = new string[] { "Account Number: ", "Amount: " };
        ...
        private void updateUserData()
        {
            ...
            if (File.Exists(path))
            {
                ...
                if (dataLine.Length >= 12)
                {
                    ...
                    dataLine[11] = today + "|Withdraw|" + ContentValue[1] + "|" + dataLine[6].Replace("Balance |", "") + "|";
                }
                else
                    dataLine = dataLine.Concat(new string[] { today + "|Withdraw|" + ContentValue[1] + "|" + dataLine[6].Replace("Balance |", "") + "|" }).ToArray();
                if (Decimal.Parse(dataLine[6].Replace("Balance |", "")) >= 0)
                {
                ...
                {
                    write.ClearAtY(8);
                    write.WriteAt("Amount is not enough", 0, 8);
                }
            }
            else
            {
